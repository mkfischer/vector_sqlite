...
class FqlDb:
    ...
    def store_to_db(self, data: List[IndexData]) -> None:
        ...
        try:
            ...
            cprint(f"Stored {len(data)} records to database table {self.index_name}.", "green")
...
def test_fql_db():
    ...
    assert retrieved_data[0][0] in ids
    assert retrieved_data[1][0] in ids
    ...
    try:
        os.remove(f"{index_name}.pkl")
        os.remove(db_name)
        cprint("Test files removed.", "yellow")
    except FileNotFoundError:
        cprint("Test files not found during cleanup.", "red")
